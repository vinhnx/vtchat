        ...
        const planResponse = await llm.chat.completions.create({
            ...
            messages: [
                ...
                },
                {
                    role: 'user',
                    content:
                        `Break down this research query into specific search tasks: "${userQuery}". Return a JSON array of tasks.`,
                },
            ],
            ...
            const searchResponse = await llm.chat.completions.create({
                ...
                messages: [
                    ...
                    },
                    {
                        role: 'user',
                        content:
                            `Search for information about: ${task}. Return relevant facts and data.`,
                    },
                ],
            });
        ...
        const analysisResponse = await llm.chat.completions.create({
            ...
            messages: [
                ...
                },
                {
                    role: 'user',
                    content:
                        `Analyze the following information regarding "${query}" and provide a coherent analysis with key insights:\n\n${
                            searchResults.join('\n\n')
                        }`,
                },
            ],
        ...
        const reportResponse = await llm.chat.completions.create({
            ...
            messages: [
                ...
                },
                {
                    role: 'user',
                    content:
                        `Create a comprehensive report on "${query}" using the following analysis and insights.\n\nAnalysis: ${analysis}\n\nInsights: ${
                            insights.join('\n- ')
