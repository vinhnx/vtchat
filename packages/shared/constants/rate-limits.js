 ...
 */
export var GEMINI_FLASH_LIMITS = GEMINI_LIMITS.FLASH_LITE;
/**
 * Helper functions for consistent UI copy formatting.
 * Keeps UI components DRY and ensures consistent messaging.
 */
export var limitText = {
    free: function (model) {
        if (model === void 0) { model = "FLASH_LITE"; }
        return "".concat(GEMINI_LIMITS[model].FREE_DAY, " requests/day, ").concat(GEMINI_LIMITS[model].FREE_MINUTE, "/min");
    },
    plus: function (model) {
        if (model === void 0) { model = "FLASH_LITE"; }
        return "".concat(GEMINI_LIMITS[model].PLUS_DAY, " requests/day, ").concat(GEMINI_LIMITS[model].PLUS_MINUTE, "/min");
    },
    compare: function (model) {
        if (model === void 0) { model = "FLASH_LITE"; }
        return "".concat(limitText.plus(model), " vs ").concat(GEMINI_LIMITS[model].FREE_DAY, "/day, ").concat(GEMINI_LIMITS[model].FREE_MINUTE, "/min");
    },
};
