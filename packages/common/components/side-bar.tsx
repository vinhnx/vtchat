...
export const Sidebar = ({ forceMobile = false }: { forceMobile?: boolean; } = {}) => {
    ...
    const sortThreads = (threads: Thread[], sortBy: 'createdAt') => {
        ...
        return [...validThreads].sort((a, b) => {
            try {
                ...
                log.error({ error, sortBy, threadA: a.id, threadB: b.id }, 'Error sorting threads');
                return 0;
            }
        });
    };
    ...
    sortThreads(threads, 'createdAt')?.forEach((thread) => {
        ...
        try {
            ...
            log.error({ error, threadId: thread.id, threadCreatedAt: thread.createdAt }, 'Error processing thread date');
        }
    });
