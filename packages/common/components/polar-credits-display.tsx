...
export function PolarCreditsDisplay() {
    ...
    // Polar.sh credits from our new store
    const { balance: polarCredits, isLoading: creditsLoading } = useCreditsStore();
    ...
    // Calculate total available credits (Polar + daily)
    const dailyCredits = creditLimit.isFetched ? creditLimit.remaining || 0 : 0;
    const totalCredits = polarCredits + dailyCredits;
                    ...
                    <div className="flex gap-1 text-xs">
                        {polarCredits > 0 && (
                            <span className="text-muted-foreground/60">
                                {polarCredits} purchased
                            </span>
                        )}
                        {polarCredits > 0 && dailyCredits > 0 && (
