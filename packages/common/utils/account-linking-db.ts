...
export async function getLinkedAccountsFromDB(userId: string): Promise<LinkedAccount[]> {
    try {
        ...
        if (!response.ok) {
            console.warn('Failed to fetch linked accounts from API:', response.status);
            return [];
        }
        ...
        console.error('Error fetching linked accounts from database:', error);
        return [];
    }
}
...
export async function getLinkedAccountsFromNeonMCP(userId: string): Promise<LinkedAccount[]> {
    ...
    try {
        ...
        console.error('[Neon MCP] Query failed:', error);
        throw error;
    }
}
...
export async function isProviderLinkedMCP(userId: string, providerId: string): Promise<boolean> {
    ...
    try {
        ...
        console.error('[Neon MCP] Existence check failed:', error);
        return false;
    }
}
