...
async function getCurrentUsage(
    ...
    const result = await tx.execute(
        sql`
            ...
            AND ${sql.identifier(VtPlusUsageColumns.PERIOD_START)} = ${periodStart.toISOString().split("T")[0]}
        `,
    );
...
async function upsertUsage(
    ...
    const result = await tx.execute(
        sql`
            ...
            VALUES (${userId}, ${feature}, ${periodStartStr}, ${amount}, NOW(), NOW())
            ON CONFLICT (
                ${sql.identifier(VtPlusUsageColumns.USER_ID)},
                ${sql.identifier(VtPlusUsageColumns.FEATURE)},
                ${sql.identifier(VtPlusUsageColumns.PERIOD_START)}
...
export async function consumeQuota(options: ConsumeOptions): Promise<void> {
    ...
    // Check if user is admin - bypass quota restrictions completely
    const adminUserIds = process.env.ADMIN_USER_IDS?.split(",").map((id) => id.trim()) || [];
    const isAdmin = adminUserIds.includes(userId);
    if (isAdmin) {
        log.info(
            { userId, feature, amount },
            "Admin user detected - bypassing quota restrictions completely",
        );
        return; // Admin users have unlimited quota
    }
    ...
    const usage = await db
        ...
        .where(
            and(
                ...
                eq(schema.vtplusUsage.periodStart, periodStartStr),
            ),
        )
        .limit(1);
...
export async function getAllUsage(userId: string): Promise<Record<VtPlusFeature, UsageResponse>> {
    ...
    for (const [window, features] of Array.from(featuresByWindow.entries())) {
        ...
        for (const feature of features) {
            const { limit } = VT_PLUS_LIMITS[feature as VtPlusFeature];
            (result as any)[feature] = {
