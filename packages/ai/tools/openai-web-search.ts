 ...
 */
export const openaiWebSearchWithModel = (modelId: string = 'gpt-4o-mini') => tool({
    description: `Search the web using OpenAI's ${modelId} model with built-in web search capabilities`,
    parameters: z.object({
        query: z.string().describe('The search query to find information about'),
        maxResults: z.number().optional().describe('Maximum number of results to return (if supported)'),
    }),
    execute: async ({ query, maxResults }) => {
        try {
            if (!supportsOpenAIWebSearch(modelId)) {
                throw new Error(`Model ${modelId} does not support OpenAI Responses API web search`);
            }
