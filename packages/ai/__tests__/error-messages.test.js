...
import { generateErrorMessage, getInvalidApiKeyError, getMissingApiKeyError, getNetworkError, getQuotaExceededError, getRateLimitError, getServiceUnavailableError, PROVIDER_DISPLAY_NAMES, PROVIDER_SETUP_URLS, } from "../services/error-messages";
describe("Error Message Service", function () {
    describe("PROVIDER_SETUP_URLS constants", function () {
        it("should have URLs for all providers", function () {
            expect(PROVIDER_SETUP_URLS[Providers.OPENAI]).toBe("https://platform.openai.com/api-keys");
            ...
            expect(PROVIDER_SETUP_URLS[Providers.LMSTUDIO]).toBe("https://lmstudio.ai/docs/local-server");
            expect(PROVIDER_SETUP_URLS[Providers.OLLAMA]).toBe("https://ollama.ai/download");
        });
    });
    describe("PROVIDER_DISPLAY_NAMES constants", function () {
        it("should have display names for all providers", function () {
    ...
    describe("getMissingApiKeyError", function () {
        it("should return generic error for unknown provider", function () {
            var result = getMissingApiKeyError({});
        ...
        it("should return provider-specific error for OpenAI", function () {
            var result = getMissingApiKeyError({ provider: Providers.OPENAI });
        ...
        it("should return special error for LM Studio", function () {
            var result = getMissingApiKeyError({ provider: Providers.LMSTUDIO });
        ...
        it("should return special error for Ollama", function () {
            var result = getMissingApiKeyError({ provider: Providers.OLLAMA });
    ...
    describe("getInvalidApiKeyError", function () {
        it("should return generic error for unknown provider", function () {
            var result = getInvalidApiKeyError({});
            expect(result.title).toBe("Invalid API Key");
            expect(result.message).toContain("invalid");
            expect(result.settingsAction).toBe("open_api_keys");
        });
        it("should detect unauthorized errors", function () {
            var result = getInvalidApiKeyError({
        ...
        it("should detect forbidden errors", function () {
            var result = getInvalidApiKeyError({
    ...
    describe("getRateLimitError", function () {
        it("should return generic rate limit error", function () {
            var result = getRateLimitError({});
            expect(result.title).toBe("Rate Limit Exceeded");
            expect(result.message).toContain("rate limit");
        });
        it("should handle VT+ user with Gemini", function () {
            var result = getRateLimitError({
        ...
        it("should handle free user with Gemini", function () {
            var result = getRateLimitError({
                provider: Providers.GOOGLE,
                hasApiKey: false,
                isVtPlus: false,
    ...
    describe("getNetworkError", function () {
        it("should detect connection refused errors", function () {
            var result = getNetworkError({
        ...
        it("should return generic network error", function () {
            var result = getNetworkError({
                provider: Providers.ANTHROPIC,
    ...
    describe("getServiceUnavailableError", function () {
        it("should detect model not found errors", function () {
            var result = getServiceUnavailableError({
                provider: Providers.OPENAI,
    ...
    describe("getQuotaExceededError", function () {
        it("should handle VT+ quota exceeded", function () {
            var result = getQuotaExceededError({
                provider: Providers.GOOGLE,
                hasApiKey: false,
                isVtPlus: true,
            });
        ...
        it("should handle free quota exceeded", function () {
            var result = getQuotaExceededError({
                provider: Providers.GOOGLE,
                hasApiKey: false,
                isVtPlus: false,
    ...
    describe("generateErrorMessage", function () {
        it("should detect missing API key errors", function () {
            var result = generateErrorMessage("API key required", {
                provider: Providers.OPENAI,
                hasApiKey: false,
            });
            expect(result.title).toBe("OpenAI API Key Required");
        ...
        it("should detect unauthorized errors", function () {
            var result = generateErrorMessage("401 unauthorized", {
                provider: Providers.ANTHROPIC,
                hasApiKey: true,
            });
        ...
        it("should detect rate limit errors", function () {
            var result = generateErrorMessage("rate limit exceeded", {
                provider: Providers.GOOGLE,
                hasApiKey: false,
                isVtPlus: false,
            });
        ...
        it("should detect network errors", function () {
            var result = generateErrorMessage("ECONNREFUSED", {
                provider: Providers.LMSTUDIO,
                hasApiKey: true,
            });
            expect(result.title).toBe("LM Studio Connection Failed");
        ...
        it("should return generic error for unknown errors", function () {
            var result = generateErrorMessage("unknown error", {});
            expect(result.title).toBe("AI Service Error");
            expect(result.message).toContain("unexpected error");
            expect(result.settingsAction).toBe("open_api_keys");
        });
        it("should handle Error objects", function () {
            var error = new Error("API key required");
            var result = generateErrorMessage(error, {
                provider: Providers.OPENAI,
                hasApiKey: false,
            });
            expect(result.title).toBe("OpenAI API Key Required");
