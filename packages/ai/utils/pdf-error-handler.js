 ...
 */
export function handlePDFProcessingError(error) {
    var errorMessage = (error === null || error === void 0 ? void 0 : error.message) || String(error);
...
export function validatePDFFile(file) {
    ...
    if (file.size > maxSize) {
        return {
            ...
            error: {
                type: "PDF_SIZE_ERROR",
                message: "File size ".concat(file.size, " exceeds maximum ").concat(maxSize),
                userMessage: "PDF file is too large",
    ...
    if (file.type !== "application/pdf") {
        return {
            ...
            error: {
                type: "PDF_FORMAT_ERROR",
                message: "Invalid MIME type: ".concat(file.type),
    ...
    if (!file.name.toLowerCase().endsWith(".pdf")) {
        return {
            ...
            error: {
                type: "PDF_FORMAT_ERROR",
                message: "Invalid file extension: ".concat(file.name),
 ...
 */
export function shouldRetryPDFProcessing(error) {
    return error.retryable && ["API_ERROR"].includes(error.type);
}
/**
 * Gets user-friendly error message for display
 */
export function getPDFErrorDisplayMessage(error) {
    switch (error.type) {
        case "PDF_NO_PAGES":
            return {
