...
const QUICK_PATTERNS = {
    ...
    documentProcessing: /upload|pdf|document|file|contract|resume|analyze.*document|parse.*file|summarize.*document/i,
    structuredOutput: /extract.*structured|extract.*json|structured.*data|key.*value|table.*data|invoice.*fields/i,
    visionAnalysis: /image|photo|picture|diagram|screenshot|analyze.*image|describe.*photo|what.*in.*picture/i
}
    ...
    execute: async ({ context, events }) => {
        ...
        try {
            ...
            if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
                window.dispatchEvent(new CustomEvent('semantic-router-result', {
                    detail: routerResult
                }))
            }
            ...
            log.error({ error, question: question.substring(0, 100) }, 'Semantic tool routing failed')
...
function tryQuickMatch(
    ...
    if (QUICK_PATTERNS.calculator.test(question)) {
        if (hasToolAccess('calculator', userTier) && !context.get('mathCalculator')) {
            ...
            matchedPattern = 'math/calculation pattern'
        }
    }
    ...
    if (QUICK_PATTERNS.webSearch.test(question)) {
        if (hasToolAccess('web-search', userTier) && !context.get('webSearch')) {
            ...
            matchedPattern = 'web search pattern'
        }
    }
    ...
    if (QUICK_PATTERNS.charts.test(question)) {
        ...
        if (hasToolAccess('charts', userTier) && !context.get('charts')) {
            tools.push('charts')
            matchedPattern = 'visualization pattern'
        }
    }
    ...
    if (QUICK_PATTERNS.documentProcessing.test(question)) {
        if (hasToolAccess('document-processing', userTier) && !context.get('documentProcessing')) {
            ...
            matchedPattern = 'document processing pattern'
        }
    }
    ...
    if (QUICK_PATTERNS.structuredOutput.test(question)) {
        if (hasToolAccess('structured-output', userTier) && !context.get('structuredOutput')) {
            ...
            matchedPattern = 'structured extraction pattern'
        }
    }
    ...
    if (QUICK_PATTERNS.visionAnalysis.test(question)) {
        if (hasToolAccess('vision-analysis', userTier) && !context.get('visionAnalysis')) {
            ...
            matchedPattern = 'image analysis pattern'
        }
    }
