...
async function testWorkflowRouting() {
    ...
    try {
        ...
        if (routedTo === "gemini-web-search") {
            console.log("  ✅ SUCCESS: Basic web search routes to gemini-web-search (bypasses planner)");
            return true;
        } else {
            console.log(`  ❌ FAILED: Basic web search routed to ${routedTo} instead of gemini-web-search`);
            return false;
        }
...
async function testProSearchRouting() {
    ...
    try {
        ...
        if (routedTo === "gemini-web-search") {
            ...
            console.log(`  ❌ FAILED: Pro Search routed to ${routedTo} instead of gemini-web-search`);
            return false;
        }
