...
async function testQuotaConsumption() {
    ...
    try {
        ...
        console.log(
            `VT+ user (${sampleVTPlusUser.planSlug}): ${
                isEligibleForQuotaConsumption(sampleVTPlusUser, false)
            }`,
        );
        console.log(
            `Free user (${sampleFreeUser.planSlug}): ${
                isEligibleForQuotaConsumption(sampleFreeUser, false)
            }`,
        );
        console.log(
            `Undefined user (${sampleUndefinedUser.planSlug}): ${
                isEligibleForQuotaConsumption(sampleUndefinedUser, false)
            }`,
        );
        ...
        if (vtPlusUsers.length > 0) {
            ...
            for (const user of vtPlusUsers.slice(0, 3)) {
                ...
                console.log(
                    `  Quota consumption eligible: ${
                        isEligibleForQuotaConsumption(userForQuota, false)
                    }`,
                );
                console.log('');
            }
