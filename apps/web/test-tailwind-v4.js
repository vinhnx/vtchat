...
function testTailwindV4() {
    ...
    const cssFile = path.join(__dirname, '../../packages/tailwind-config/test-output-with-source.css');
    ...
    const tests = [
        ...
        },
        {
            ...
            selector: '.ring-3',
            expected: '--tw-ring-shadow: var(--tw-ring-inset,) 0 0 0 calc(3px + var(--tw-ring-offset-width))',
            breaking: false
        },
        {
    ...
    tests.forEach(test => {
        ...
        if (found) {
            ...
            const selectorMatch = css.match(new RegExp(`${test.selector.replace('.', '\\.')}\\s*{[^}]+}`, 'g'));
            if (selectorMatch) {
