...
describe('Address Masking', () => {
    it('should mask complete address information', () => {
        const content = 'Please send mail to: John Doe, 123 Main Street, Apt 4B, San Francisco, CA 94102';
        const result = secureContentForEmbedding(content);
        expect(result).toBe('Please send mail to: John Doe, [ADDRESS_REDACTED], [UNIT_REDACTED], San Francisco, CA [ZIP_REDACTED]');
    });
