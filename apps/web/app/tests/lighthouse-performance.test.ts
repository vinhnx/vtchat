...
test.describe('Lighthouse Performance Optimizations', () => {
    ...
    test('Performance optimizations component loaded', async ({ page }) => {
        ...
        const dnsPreconnect = await page.locator('link[rel="dns-prefetch"][href*="google.com"]').count();
    ...
    test('No duplicate font preconnects', async ({ page }) => {
        ...
        const googleFontsPreconnects = await page.locator('link[rel="preconnect"][href*="fonts.googleapis.com"]').count();
        const googleStaticPreconnects = await page.locator('link[rel="preconnect"][href*="fonts.gstatic.com"]').count();
    ...
    test('Image optimization works', async ({ page }) => {
        ...
        const backgroundImage = page.locator('img[src*="/bg/bg_vt.avif"], img[src*=".avif"], img[src*=".webp"]');
    ...
    test('JavaScript bundles are optimized', async ({ page }) => {
        ...
        const hasVendorChunk = jsRequests.some(url => url.includes('vendor') || url.includes('chunk'));
    ...
    test('Critical CSS loads first', async ({ page }) => {
        ...
        const resourceTimings = await page.evaluate(() => {
            return performance.getEntriesByType('resource').map((entry: any) => ({
                ...
                type: entry.name.includes('.css') ? 'css' : entry.name.includes('.js') ? 'js' : 'other'
            }));
        });
        
        const cssLoadTime = Math.min(...resourceTimings.filter(r => r.type === 'css').map(r => r.startTime));
        const jsLoadTime = Math.min(...resourceTimings.filter(r => r.type === 'js').map(r => r.startTime));
