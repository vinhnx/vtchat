...
describe('Embedding Security Integration', () => {
    ...
    it('should handle mixed PII content', async () => {
        const mixedContent = 'My email is john@test.com, phone is 555-123-4567, and my card is 1234 5678 9012 3456';
        const securedContent = secureContentForEmbedding(mixedContent);
        
        expect(securedContent).toBe('My email is [EMAIL_REDACTED], phone is [PHONE_REDACTED], and my card is [CARD_REDACTED]');
    });
});
