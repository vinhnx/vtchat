...
async function getComprehensiveSubscriptionStatus(userId: string) {
    try {
        ...
        const subscriptionSync = await import('../../../../../packages/shared/utils/subscription-sync');
        return await subscriptionSync.getComprehensiveSubscriptionStatus(userId);
    } catch (importError) {
        console.warn('Could not import subscription-sync, falling back to direct database query:', importError);
        ...
        try {
            ...
            if (subscription) {
                ...
                plan = isActive && subscription.plan === PlanSlug.VT_PLUS ? PlanSlug.VT_PLUS : PlanSlug.VT_BASE;
