...
import { type NewUserSubscription, users, userSubscriptions } from '@/lib/database/schema';
    ...
    static async upsertSubscription(
        ...
        return await db.transaction(async (tx) => {
            try {
                ...
                const planSlug = subscriptionData.plan === PlanSlug.VT_PLUS
                    ? PlanSlug.VT_PLUS
                    : PlanSlug.VT_BASE;
                await tx
                    .update(users)
    ...
    static async validateUserSubscriptions(userId: string): Promise<{
        ...
        try {
            ...
            if (user.length > 0 && activeSubscriptions.length > 0) {
                ...
                const expectedPlanSlug = subscriptionPlan === PlanSlug.VT_PLUS
                    ? PlanSlug.VT_PLUS
                    : PlanSlug.VT_BASE;
    ...
    static async fixUserSubscriptions(userId: string): Promise<void> {
        ...
        await db.transaction(async (tx) => {
            ...
            if (activeSubscriptions.length > 1) {
                ...
                const planSlug = keepSubscription.plan === PlanSlug.VT_PLUS
                    ? PlanSlug.VT_PLUS
                    : PlanSlug.VT_BASE;
                await tx
                    .update(users)
