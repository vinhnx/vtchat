...
export function SemanticRouterDebug() {
    ...
    useEffect(() => {
        ...
        return () => {
            window.removeEventListener('semantic-router-result', handleRouterEvent as EventListener)
        }
    }, [])
            ...
            <CardContent className="space-y-2">
                {routerResults.length === 0 ? (
                    ...
                    routerResults.map((result) => (
                        <div key={result.timestamp} className="p-2 border rounded text-xs space-y-1">
                            ...
                            {result.scores && result.scores.length > 0 && (
                                <div>
                                    ...
                                    {result.scores.slice(0, 2).map(s => 
                                        `${s.name}: ${(s.score * 100).toFixed(1)}%`
                                    ).join(', ')}
                                </div>
                            )}
                        </div>
